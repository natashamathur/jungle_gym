if __name__ == "__main__":
    ...
    names_to_check["full_name"] = names_to_check.firstname + " " + names_to_check.lastname

    # Run names through wikipedia API
    wiki_dict = run_wikipedia_api(names_to_check)

    # Applies wikpedia results to original dataframe
    names_to_check["wiki_name"] = names_to_check.email.apply(lambda x: get_wiki_info(x, "name", wiki_dict))
    names_to_check["wiki_bio"] = names_to_check.email.apply(lambda x: get_wiki_info(x, "bio", wiki_dict))
